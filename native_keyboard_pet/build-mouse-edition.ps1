...
$build = Join-Path $root 'build-mouse-edition'
...
Push-Location $root
try {
    & python 'tools\process_selected_mouse_sheet.py'
    if ($LASTEXITCODE -ne 0) {
        throw "Mouse frame processing failed with exit code: $LASTEXITCODE"
    }
    ...
    $app = Join-Path $build 'flame-keyboard-pet-mouse-edition.exe'
    & $compiler `
        -std=c++17 `
        -DFLAME_MOUSE_EDITION `
        -Os `
        -s `
        -static-libgcc `
        -static-libstdc++ `
    ...
    & $compiler -std=c++17 -O2 -DFLAME_MOUSE_EDITION -Wall -Wextra -Wpedantic `
    ...
    & $compiler -std=c++17 -O2 -municode -DFLAME_MOUSE_EDITION `
        -Wall -Wextra -Wpedantic 'tests\keyboard_integration_test.cpp' `
        -o $integrationTest -luser32
    if ($LASTEXITCODE -ne 0) {
    ...
    Get-ChildItem -LiteralPath (Join-Path $root 'assets\mouse-edition') -Filter '*.png' |
        Copy-Item -Destination $actionBuild -Force
    ...
    $package = Join-Path $root 'release\FlameKeyboardPet-Mouse-Edition'
    ...
    Get-ChildItem -LiteralPath $actionBuild -Filter '*.png' |
        Copy-Item -Destination $packageActions -Force
    Copy-Item -LiteralPath 'packaging\launch-mouse-edition.bat' `
        -Destination (Join-Path $package 'START.bat') -Force
    Copy-Item -LiteralPath 'packaging\enable-startup-mouse-edition.ps1' `
        -Destination (Join-Path $package 'enable-startup.ps1') -Force
    Copy-Item -LiteralPath 'packaging\disable-startup-mouse-edition.ps1' `
        -Destination (Join-Path $package 'disable-startup.ps1') -Force
    Copy-Item -LiteralPath 'packaging\enable-startup-mouse-edition.bat' `
        -Destination (Join-Path $package 'Enable-Startup.bat') -Force
    Copy-Item -LiteralPath 'packaging\disable-startup-mouse-edition.bat' `
        -Destination (Join-Path $package 'Disable-Startup.bat') -Force
    Copy-Item -LiteralPath 'packaging\USER_GUIDE_MOUSE_EDITION.md' `
        -Destination (Join-Path $package 'README.md') -Force

    $appHash = (Get-FileHash -LiteralPath $app -Algorithm SHA256).Hash
    "$appHash *flame-keyboard-pet-mouse-edition.exe" |
        Set-Content -LiteralPath (Join-Path $package 'SHA256SUMS.txt') -Encoding ascii
    $zip = Join-Path $root 'release\FlameKeyboardPet-Mouse-Edition-Windows-x64.zip'
